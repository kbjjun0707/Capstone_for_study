...
void AUX_RGBImageRec::convertBGRAtoRGBA()
{
...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
	...
	const DWORD BitmapLength = sizeX * sizeY * (BMInfo.bmiHeader.biBitCount/8);	// 3 bytes (BGR) per pixel (24bp)  
	...
	if (GetDIBits(gldc, (HBITMAP)DIBHandle, 0, sizeY, data, &BMInfo, BMInfo.bmiHeader.biClrUsed)) 
	{
		NoErrors = true;
		if ((BMInfo.bmiHeader.biBitCount / 8) == 3)
